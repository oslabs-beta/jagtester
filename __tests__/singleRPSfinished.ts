...
jest.mock('node-fetch', () =>
	jest.fn(() => {
		return Promise.resolve({
			json: () =>
				Promise.resolve(
					globalVariables.pulledDataFromTest[globalVariables.currentInterval]
				),
		});
	})
);
...
describe('Testing singleRPSfinished functionality', () => {
	...
	it('should call emit singleRPS finished with rpsGroup', () => {
		expect(io.emit).toBeCalledWith(
			ioSocketCommands.singleRPSfinished,
			globalTestConfig.rpsInterval
		);
	});
	...
	it('should call fetch with correct header jagtester command and return correct response data', async () => {
		...
		expect(globalVariables.pulledDataFromTest[globalVariables.currentInterval]).toEqual(
			originalPulledData[globalVariables.currentInterval]
		);
	});
});
