...
var getMiddleware = function (app) {
    ...
    var newLayerHandleRequest = function handle(req, res, next) {
        ...
        try {
            var fnName = this.name, reqId_1 = req.headers.jagtesterreqid
                ? req.headers.jagtesterreqid.toString()
                : undefined, reqRoute_1 = req.originalUrl;
            ...
            fn(req, res, function () {
                if (reqId_1 && routeData[reqRoute_1] && routeData[reqRoute_1][reqId_1]) {
                    var lastElIndex = routeData[reqRoute_1][reqId_1].middlewares.length - 1;
                    routeData[reqRoute_1][reqId_1].middlewares[lastElIndex].elapsedTime =
